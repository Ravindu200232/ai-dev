# Builder pipeline orchestration, updates, and repair coordination.
def run_agent_pipeline(prompt: str, model: str, think: bool = False,
                       qa_model: str = "", resume_project: str = "",
                       logo: str = "", srs_id: str = "", theme: str = "",
                       theme_html: str = "", theme_page: str = "",
                       theme_dir: str = "", uploads: dict = None,
                       planner_model: str = "", design_model: str = ""):
    """Raw prompt → LLM plan.md → LLM writes every file in one continuous pass."""
    cancel.begin()
    # Exhaustion is per project run; never leak a failed repair into the next
    # app, but do carry it across this run's analyzer/QA stages.
    _EXHAUSTED_FINDINGS.clear()
    ...
    try:
        if resume_project:
            ...
            if not (proj_dir / ".agentforge" / "plan.json").is_file():
                ...
                planner_model = (planner_model
                                 or intent.get("planner_model", ""))
                design_model = (design_model
                                or intent.get("design_model", ""))
        ...
        design_model = design_model or model
        elog("INFO", "━" * 40)
        elog("INFO", f"🤖 Agent mode — {prompt[:80]}")
        elog("INFO", f"🧭 Planner: {planner_model}   ctx "
                     f"{max_context(planner_model):,}   ⚡ thinking off")
        elog("INFO", f"🎨 Design: {design_model}   ctx "
                     f"{max_context(design_model):,}   ⚡ thinking off")
        elog("INFO", f"🔨 Builder: {model}   ctx {max_context(model):,}"
        ...
        save_run_intent(proj_dir, prompt=prompt, model=model, think=think,
                        planner_model=planner_model, design_model=design_model,
        ...
        eprog("Checking models…", 2)
        checked = set()
        for role, chosen in (("planner", planner_model),
                             ("design", design_model),
                             ("builder", model)):
            if chosen in checked:
                continue
            checked.add(chosen)
            if not ensure_model(chosen):
                eerr(f"Cannot load {role} model: {chosen}")
                return
        ...
        qa = QASession(proj_dir, callbacks=_qa_callbacks(), model=qa_model,
                       enabled=is_cloud_model(qa_model), reasoning=bool(think))
        if qa.enabled:
            elog("INFO", f"   🧪 QA: {qa_model}   "
                         + ("🤔 thinking on" if qa.reasoning else
                            "⚡ thinking off"))
        if not qa.enabled:
        ...
        try:
            ...
            _park_exhausted(getattr(analyzer, "_exhausted_findings", []))
        except Exception as e:
        ...
        try:
            ...
            pass
        for chosen in {model, planner_model, design_model}:
            stop_model(chosen)
        cancel.finish()


def _open_for_edit(proj_name: str, model: str, think: bool = False):
    ...
    if stack != "next":
        eerr("Only Next.js projects can be edited by the builder")
        return None, None, None
