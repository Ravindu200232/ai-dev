# Design-agent generation and approved asset staging.
# HTML is a direct rendering task. Long hidden reasoning and 15-minute calls
# make large cloud models slower without improving the preview.
THEME_TIMEOUT_S = 180
THEME_STALL_S = 120
THEME_OUTPUT_TOKENS = 6_000
THEME_RETRY_ATTEMPTS = 2


def _generate_design_html(direction, brief, app_name, model):
    """Stream one bounded HTML answer so a timed-out tail is still usable."""
    chunks = []
    total = int(direction.get("total") or themekit.COUNT)
    for chunk in ollama.chat_stream(
            model,
            [{"role": "system", "content": themekit.SYSTEM},
             {"role": "user", "content": themekit.user_prompt(
                 brief, app_name, direction, direction["index"], total)}],
            options={"temperature": 0.75,
                     "num_predict": THEME_OUTPUT_TOKENS},
            think=False,
...
def _draw_design(direction, brief, app_name, model, settings, out, lock,
    ...
    try:
        r = with_retry(
            lambda: _generate_design_html(direction, brief, app_name, model),
            what=f"the design model (direction {k})",
            attempts=THEME_RETRY_ATTEMPTS)
        raw = str(r or "").strip()
    ...
    if not html:
        ...
        return
    preview_pages = direction.get("preview_pages") or []
    if preview_pages:
        html = themekit.align_preview_sitemap(html, preview_pages)
        site_issue = themekit.sitemap_issue(html, preview_pages)
        if site_issue:
            elog("WARN", f"   ⚠ design {k} — sitemap contract failed: "
                         f"{site_issue}; skipping this inconsistent direction")
            if why is not None:
                with lock:
                    why.append({"index": k, "busy": False,
                                "reason": "sitemap contract: " + site_issue})
            return
