# Feature-domain changes, image edits and scoped updates.
FEATURE_UNIT_REPAIR_ROUNDS = 2


def run_feature(proj_name: str, request: str, model: str, think: bool = False,
    ...
    try:
        ...
        _feature_tests(arch, proj_dir, spec, model, qa_model, think=think,
                       build_ok=True)
...
def _feature_tests(arch, proj_dir: Path, spec, model: str, qa_model: str, *,
                   build_ok: bool, think: bool = False):
    ...
    if not harness.install():
        ...
        return qa
    try:
        elog("INFO", f"   🧪 Feature unit repair budget: "
                     f"{FEATURE_UNIT_REPAIR_ROUNDS} rounds")
        run_qa_unit_stage(arch, proj_dir, qa, build_ok=True,
                          scope=spec.written,
                          max_repair_rounds=FEATURE_UNIT_REPAIR_ROUNDS)
    except Exception as e:
