...
class BugFixerPromptMixin:
    ...
    @staticmethod
    def _mechanical_failure_notes(failures, target: str) -> list[str]:
        """High-confidence test mechanics the model should not rediscover."""
        blob = "\n".join(
            str(getattr(f, "message", "") or "") + "\n" +
            str(getattr(f, "stack", "") or "") for f in (failures or []))
        notes = []
        if re.search(r"\bObjectId\s+is\s+not\s+defined\b", blob, re.I):
            notes.append(
                "`ObjectId` is undefined in the TEST. Replace "
                "`new ObjectId(value)` with the supported `oid(value)` helper; "
                "do not invent a local ObjectId function or change the route.")
        if re.search(r"role\s+[\"']?form|role \"form\"", blob, re.I):
            notes.append(
                "A bare `<form>` has no accessible `form` role. Use the render "
                "container's `querySelector('form')` with `fireEvent.submit`, "
                "unless the source gives the form an aria-label.")
        if "/route." in (target or "") and re.search(
                r"\[(?:\.\.\.)?[A-Za-z_$][\w$]*\]", target or ""):
            notes.append(
                "This is a dynamic App Router handler. The URL alone does not "
                "populate route context; every request-helper call must pass "
                "the named value in `{ params: { ... } }`.")
        return notes
    ...
    def _prompt(self, failures, test_body, target_body, forced, reason,
        ...
        f0 = failures[0]
        cases = []
        for index, f in enumerate(failures[:6]):
            frames = [ln for ln in (f.stack or "").splitlines()
                      if ln.strip().startswith("at ")][:4]
            rendered = ""
            if index < 2 and getattr(f, "dom", ""):
                rendered = ("\n    Rendered DOM at failure:\n" +
                            "\n".join(f"      {ln}" for ln in
                                      str(f.dom).splitlines()[:35]))
            hint = ""
            if getattr(f, "hint", ""):
                hint = "\n" + "\n".join(
                    f"    {ln}" for ln in str(f.hint).splitlines()[:20])
        ...
        if mechanics:
            parts.append("## Mechanical evidence already established\n" +
                         "\n".join(f"  • {note}" for note in mechanics))
        ...
        if refused:
            parts.append(
                ...
                f"again.")
        if tier >= 2 and target_body and not forced:
            parts.append(
