    ...
    def _normalise_plan(self, data: dict) -> dict:
        ...
        for a in data.get("demo_accounts") or []:
            if isinstance(a, dict) and a.get("email") and a.get("password"):
                accounts.append({
                    "email": str(a["email"]).strip(),
                    "password": a["password"],
                    "role": str(a.get("role") or "user").strip() or "user",
                })

        # In an RBAC app an email is an identity, not a reusable fixture label.
        # A planner occasionally assigns the same address to several roles. The
        # seed then upserts by email and the last role silently wins, making (for
        # example) an owner authenticate as a customer. Canonicalise every role,
        # whatever it is called, to one distinct identity. A single-role/auth-free
        # plan is intentionally left alone.
        role_keys = {a["role"].casefold() for a in accounts if a.get("role")}
        if len(role_keys) >= 2:
            used_emails, used_roles, isolated = set(), set(), []
            for account in accounts:
                role_key = account["role"].casefold()
                if role_key in used_roles:
                    continue

                email = account["email"].casefold()
                if email in used_emails:
                    role_slug = re.sub(r"[^a-z0-9]+", ".", role_key).strip(".")
                    role_slug = role_slug or "user"
                    domain = email.rsplit("@", 1)[-1] if "@" in email else "demo.com"
                    if "." not in domain:
                        domain = "demo.com"
                    email = f"{role_slug}@{domain}"
                    suffix = 2
                    while email in used_emails:
                        email = f"{role_slug}{suffix}@{domain}"
                        suffix += 1

                isolated.append({**account, "email": email})
        ...
        for w in data.get("workflows") or []:
            ...
            workflows.append({
                ...
                # A plan may carry up to 40 capabilities. Truncating this at
                # 20 silently turns CAP-021+ into "unwalked" capabilities even
                # when the model covered them correctly.
                "covers": covers[:40],
                "steps": steps[:12],
            })
