...
have to remember what an earlier file meant. Use one contract for every:
  ...
  • client-to-API mutation/read: `kind=api`, `from` is the page/component
    ...
    success destination/UI change. For every non-navigation command, the
    effect also names the exact success toast and error toast; success is
    emitted only after the promised call/computation really completes.
...
ACTION FEEDBACK IS PART OF THE PLAN, NOT A BUILDER GUESS. Every user-triggered
operation that completes work or can fail — create, edit, delete, submit,
login, signup, logout, upload/import/export, download/copy, calculate,
generate and any async command — has BOTH an exact success-toast message and
an exact error-toast message in the owning `actions` entry. Name when each is
emitted. Never show success optimistically before persistence or completion,
and never substitute a toast for the resulting row, value, download,
destination or other durable visible proof. Pure navigation, tabs, filters
and disclosure toggles use their destination/state as feedback and do not
spam a toast. These transient in-app toasts are operational UI feedback; they
do not invent the email/SMS/inbox `notification_rules` business feature that
the request may not have asked for.
...
into an app, and they ride into the very turn that writes the file:
  • THE NAVBAR, on every page except the sign-in ones if this app has
    ...
    `<body>` and `{children}` (optionally wrapped by ToastProvider), and no chrome,
    so a page that does not name
...
literal `data-testid` belongs. For a non-navigation operation, that owning
action also names its success/error toasts, and the workflow effect says the
success toast is observed before the durable state/destination proof. A
workflow is invalid if its button exists only in prose.
...
PLANNING RULES:
  ...
  • NO ROUTE GROUPS. Never plan a folder in parentheses: not
    ...
    only `<html>`, `<body>` and `{children}` (optionally wrapped by ToastProvider),
    and never a navbar, so
  ...
  • WHEN THE APP STORES ANYTHING, the plan MUST include `lib/seed.js` and
    ...
    it being luck.
    ACTION FEEDBACK HAS ONE SHARED OWNER. If the app has any non-navigation
    action, plan `components/ToastProvider.jsx` once as a client component
    with a toast host plus a small context/hook API, accessible live regions
    (`role=status` for success, `role=alert` for errors), deduplication and
    dismissal. Put it in task 1 before any consumer, and list `app/layout.jsx`
    in that same task to import it and wrap `{children}`. That wrapper is
    structural plumbing, not app chrome, and it must not render the navbar on
    auth pages. Use this local component without adding a package unless the
    approved design already requires a toast dependency.
  ...
  • SIGN-IN IS NOT A DEFAULT. Plan it only if the brief has people who sign
    ...
    LOGOUT HAS A REQUIRED DESTINATION TOO. When the app exposes a sign-out/
    logout control, its planned action must await `signOut()`, replace the
    current route with `/login`, and refresh the router so protected page
    state cannot remain visible. Add an E2E workflow step that clicks logout
    and proves the browser lands on `/login`; merely clearing the session is
    not a complete logout experience.
