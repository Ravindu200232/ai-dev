...
THE REQUEST DECIDES. It is the floor and it is the ceiling.
  ...
  • Build nothing it did not ask for. Every Core Feature bullet traces to
    ...
    reviews or outbound notifications (email, SMS, inboxes, notification
    centres) that nobody asked for is not a better plan; it is a different
    app, and the person now has to use it. Transient in-app toast feedback is
    not a new requested feature: it is the completion/error state of an action.
...
with a menu of pages nobody can find their way through.
  ...
  • AFTER SOMEONE DOES THE THING, THEY SEE THAT IT HAPPENED. The row in the
    list, the new total, the page they land on. Every non-navigation action
    also has a transient success toast after confirmed completion and an
    error toast when it fails. A form that submits into
    silence is the most common way a working app feels broken; a toast alone
    is not proof that its data persisted.
...
OUTPUT BUDGET — keep the WHOLE answer under 45,000 characters, even for a long
specification. The JSON is authoritative: keep prose concise, do not repeat a
requirement in several sections, and reserve enough space to close the complete
JSON object and fence. More words are not a more complete plan.
...
```json
{
  ...
  "capabilities": [
    ...
    {"id": "CAP-002", "who": "<<actor>>",
     ...
     "proof": "submit persists a <<child>>, shows the planned success toast, and the destination shows its id/status",
  ...
  "workflows": [
    {"name": "Main journey", "who": "<<actor>>", "covers": ["CAP-001", "CAP-002"],
     "steps": ["/ — click '<<Things>>' — the <<thing>> grid",
               ...
               "/<<children>>/new — submit it — success toast then /<<children>>/[id] showing the <<child>>"]},
  ...
  "contracts": [
    {"name": "create-<<child>>", "kind": "api",
     ...
     "effect": "on success persist the <<child>>, show '<<Child>> created', then navigate to /<<children>>/[id]; on failure show 'Could not create <<child>>'"},
      ...
      "files": [
        ...
        {"path": "components/TaskList.js", "kind": "client",
          ...
          "actions": ["the search box filters the list as you type — testid=task-search",
                      "each row's toggle calls PATCH /api/tasks/[id], then updates in place and shows success toast 'Task updated'; failure toast 'Could not update task'"]}
      ]
    }
  ]
}
...
form: the proof includes persistence, the planned success/error toast, and a
reload showing the new price. A toast is outcome feedback, never the sole
business proof.
