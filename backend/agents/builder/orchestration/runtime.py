"""Next.js builder initialization, streaming, and conversation startup."""
from .common import *
...
class ArchitectRuntimeMixin:
    def __init__(self, client: OllamaClient, model: str, project_dir: Path,
                 ...
                 think: bool = False, theme_html: str = "",
                 theme_page: str = "", planner_model: str = "",
                 design_model: str = ""):
        self.client = client
        self.model = model
        self.planner_model = planner_model or model
        self.design_model = design_model or model
        self.project_dir = Path(project_dir)
        self.cb = callbacks or {}
        if stack != "next":
            raise ValueError("Only Next.js builder projects are supported")
        self.stack = "next"
        ...
        self.planner_num_ctx = max_context(self.planner_model)
        self.planner_is_cloud = is_cloud_model(self.planner_model)
        self.design_num_ctx = max_context(self.design_model)
        self.design_is_cloud = is_cloud_model(self.design_model)
        # Context windows for models borrowed for a single call
        self._ctx_cache = {
            name: max_context(name)
            for name in {model, self.planner_model, self.design_model} if name
        }

        # Thinking is opt-in.  Only the shared Builder/QA switch should turn
        # it on; an omitted or legacy null value must keep it off.
    ...
    def vocab(self):
        ...
        from agents.planner.app_vocab import derive
    ...
    def _fit_vocab(self, text: str) -> str:
        from agents.planner.app_vocab import render
        try:
    ...
    def _builder_sys(self) -> str:
        base = self._fit_vocab(self._P["builder"]).replace(
            "{stack}", self._P["rules"])
        try:
            from agents.core.source_guidance import HUMAN_COMMENT_POLICY
        ...
        try:
            from agents.core.agent_memory import memory_for
        ...
        try:
            from agents.core.workspace import TOOL_HELP
            base += "\n\n    " + TOOL_HELP.replace("\n", "\n    ")
        except Exception as e:  # noqa: BLE001
    ...
    def _stream(self, messages, on_delta, tools=None, temperature=0.6,
                model=None, timeout=None, max_output_tokens=None,
                reasoning=None):
    ...
    def _stream_once(self, messages, on_delta, *, tools, temperature, model,
                     timeout, think, max_output_tokens=None):
        ...
        if self.theme_html:
            ...
            try:
                r = self.client.chat(
                    ...
                    options={"temperature": 0.2}, timeout=240, think=False)
