"""Atomic Next.js file writes, role coverage, and route tracking."""
from .common import *
...
class ArchitectWriteMixin:
    def write_file(self, rel: str, content: str) -> bool:
        try:
            ...
            previous = self.files.get(key0)
            if previous is None and fp.is_file():
                try:
                    previous = fp.read_text(encoding="utf-8")
                except OSError:
                    previous = None
            if previous is not None and previous.rstrip() + "\n" == content:
                # Repeating a complete file in a later repair stage is not a
                # write. Do not emit a file event or invalidate its unit test.
                return False
