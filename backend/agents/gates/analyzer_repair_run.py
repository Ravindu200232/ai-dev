...
class AnalyzerRepairRunMixin:
    def run(self, *, use_model: bool = True, max_rounds: int = 2,
        ...
        def park_survivors(attempted, current):
            attempted_keys = {finding_identity(f) for f in attempted or []}
            existing = {finding_identity(f)
                        for f in self._exhausted_findings}
            for finding in current or []:
                key = finding_identity(finding)
                if key in attempted_keys and key not in existing:
                    self._exhausted_findings.append(finding)
                    existing.add(key)
        ...
        while use_model and structural_targets(report) and rounds < max_rounds:
            ...
            n = self.repair(focus)
            if not n:
                park_survivors(targets, structural_targets(report))
                break
            total_written += n
