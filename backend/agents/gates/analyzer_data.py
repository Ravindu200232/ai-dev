...
class AnalyzerDataMixin:
    ...
    SEED_ACCOUNT_OBJECT_RE = re.compile(
        r"\{(?=[^{}]{0,500}\bemail\s*:\s*['\"])(?=[^{}]{0,500}"
        r"\brole\s*:\s*['\"])[^{}]{1,500}\}", re.S)
    SEED_EMAIL_FIELD_RE = re.compile(
        r"\bemail\s*:\s*['\"]([^'\"]+)['\"]", re.I)
    SEED_ROLE_FIELD_RE = re.compile(
        r"\brole\s*:\s*['\"]([^'\"]+)['\"]", re.I)

    def role_identity_collisions(self) -> list:
        """A multi-role seed must not collapse several roles into one user."""
        plan = getattr(self.arch, "plan", None) or {}
        planned_roles = {
            str(a.get("role") or "").strip().casefold()
        ...
        for path, content in sorted(self.code_files().items()):
            if "seed" not in path.lower():
                continue
            roles_by_email = {}
            for match in self.SEED_ACCOUNT_OBJECT_RE.finditer(content):
                block = match.group(0)
                email_match = self.SEED_EMAIL_FIELD_RE.search(block)
                role_match = self.SEED_ROLE_FIELD_RE.search(block)
                if not email_match or not role_match:
                    continue
                email = email_match.group(1).strip().casefold()
                role = role_match.group(1).strip().casefold()
                if email and role:
                    roles_by_email.setdefault(email, set()).add(role)

            collisions = {
                email: roles for email, roles in roles_by_email.items()
                if len(roles) > 1
            }
            if not collisions:
                continue
            email, roles = next(iter(collisions.items()))
            out.append(Finding(
                "blocker", "ROLE_IDENTITY_COLLISION",
                f"the multi-role seed assigns `{email}` to several roles "
                f"({', '.join(sorted(roles))}); because auth and role updates "
                "key on email, the last seed update overwrites the same user "
                "and the roles authenticate as one another",
                path=path,
                fix="give every planned role its own email/user document, then "
                    "resolve persisted users by email into a role map and attach "
                    "owned seed rows to the matching role identity"))
        return out
