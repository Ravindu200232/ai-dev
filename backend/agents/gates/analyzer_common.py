...
def finding_identity(finding, limit: int = 260) -> tuple:
    """Stable identity for carrying one failed repair across pipeline stages."""
    return (
        str(getattr(finding, "code", "") or ""),
        str(getattr(finding, "path", "") or ""),
        re.sub(r"\b[0-9a-f]{24}\b", "<id>",
               str(getattr(finding, "message", "") or ""))[:limit],
    )
